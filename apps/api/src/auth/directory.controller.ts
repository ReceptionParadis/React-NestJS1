...
export class DirectoryController {
  ...
  async directory(@Headers('authorization') authorization?:string){
    ...
    return users.filter(user=>!user.email.endsWith('@hospicore.invalid')).map(user=>({id:user.id,firstName:user.firstName,lastName:user.lastName,role:user.role.name}));
  }
}
