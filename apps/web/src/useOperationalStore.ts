...
export function useOperationalStore<T>(namespace:string,initialValue:T,refreshMs=DEFAULT_REFRESH_MS):OperationalStore<T>{
 const initialValueRef=useRef(initialValue),[data,setData]=useState<T>(()=>initialValueRef.current),dataRef=useRef<T>(initialValueRef.current),dataSignatureRef=useRef(stableSignature(initialValueRef.current)),[version,setVersion]=useState(0),versionRef=useRef(0),refreshInFlightRef=useRef(false),writeEpochRef=useRef(0),mountedRef=useRef(true),channelRef=useRef<BroadcastChannel|null>(null),pendingPayloadRef=useRef<T|null>(null),saveTimerRef=useRef<number|null>(null),[updatedAt,setUpdatedAt]=useState(''),[lastSuccessAt,setLastSuccessAt]=useState(''),[state,setState]=useState<OperationalSyncState>('loading'),[message,setMessage]=useState('Connexion à PostgreSQL…');
 ...
 const save=useCallback(async(next:T):Promise<boolean>=>{if(namespace==='group-360'){const protectedNext=protectGroup360(dataRef.current,next) as T;if(statusChanged(dataRef.current,protectedNext))return saveImmediate(protectedNext);pendingPayloadRef.current=protectedNext;if(mountedRef.current){applyPayload(protectedNext);setState('synced');setMessage('Saisie enregistrée automatiquement…');}emitOperationalDraft({namespace,payload:protectedNext});if(saveTimerRef.current!==null)window.clearTimeout(saveTimerRef.current);saveTimerRef.current=window.setTimeout(async()=>{const payload=pendingPayloadRef.current;if(payload===null)return;pendingPayloadRef.current=null;saveTimerRef.current=null;await persist(payload,'Fiche Groupe 360° enregistrée');},GROUP_360_SAVE_DELAY);return true;}return persist(next,'Enregistré pour tous les services');},[applyPayload,namespace,persist,saveImmediate]);
