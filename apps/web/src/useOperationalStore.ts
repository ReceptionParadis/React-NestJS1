...
export function useOperationalStore<T>(namespace:string,initialValue:T,refreshMs=DEFAULT_REFRESH_MS):OperationalStore<T>{
 const initialValueRef=useRef(initialValue),[data,setData]=useState<T>(()=>initialValueRef.current),dataRef=useRef<T>(initialValueRef.current),dataSignatureRef=useRef(stableSignature(initialValueRef.current)),[version,setVersion]=useState(0),versionRef=useRef(0),refreshInFlightRef=useRef(false),mountedRef=useRef(true),channelRef=useRef<BroadcastChannel|null>(null),[updatedAt,setUpdatedAt]=useState(''),[lastSuccessAt,setLastSuccessAt]=useState(''),[state,setState]=useState<OperationalSyncState>('loading'),[message,setMessage]=useState('Connexion à PostgreSQL…');
 ...
 const refresh=useCallback(async(silent=false):Promise<boolean>=>{if(namespace==='group-360'&&group360PendingPayload!==null)return true;if(refreshInFlightRef.current)return false;refreshInFlightRef.current=true;if(!silent&&mountedRef.current){setState('loading');setMessage('Synchronisation en cours…');}try{const previousVersion=versionRef.current,result=await loadSharedData<T>(namespace,initialValueRef.current);if(!mountedRef.current)return false;if(!result.connected){setState('error');setMessage(result.error||'Synchronisation PostgreSQL indisponible.');return false;}markSuccess(result.payload,result.version,'Données partagées à jour');if(silent&&previousVersion>0&&result.version>previousVersion)emitOperationalChange({namespace,version:result.version,at:Date.now(),source:'remote'});return true;}catch(error){if(!mountedRef.current)return false;setState('error');setMessage(error instanceof Error?error.message:'Synchronisation PostgreSQL indisponible.');return false;}finally{refreshInFlightRef.current=false;}},[markSuccess,namespace]);
 const persist=useCallback(async(next:T,successMessage:string):Promise<boolean>=>{if(mountedRef.current){applyPayload(next);setState('saving');setMessage('Enregistrement partagé…');}emitOperationalDraft({namespace,payload:next});try{const result=await saveSharedData<T>(namespace,next,versionRef.current);if(!mountedRef.current)return false;markSuccess(result.payload,result.version,successMessage);const detail:OperationalChangeDetail={namespace,version:result.version,at:Date.now(),source:'local'};emitOperationalChange(detail);channelRef.current?.postMessage(detail);return true;}catch(error){if(!mountedRef.current)return false;const text=error instanceof Error?error.message:'Enregistrement impossible.';const conflict=text.includes('autre personne');setState(conflict?'conflict':'error');setMessage(text);if(conflict)window.setTimeout(()=>void refresh(true),250);return false;}},[applyPayload,markSuccess,namespace,refresh]);
 ...
 const save=useCallback(async(next:T):Promise<boolean>=>{
  if(namespace==='group-360'){
   if(statusChanged(dataRef.current,next))return saveImmediate(next);
   group360PendingPayload=next;
   if(mountedRef.current){applyPayload(next);setState('synced');setMessage('Saisie enregistrée automatiquement…');}
   emitOperationalDraft({namespace,payload:next});
