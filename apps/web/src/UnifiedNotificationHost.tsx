...
function dedupe(items:ImportantNotification[]){const seen=new Set<string>();return items.filter(item=>{const explicit=item.id.startsWith('cash-validation-')||item.id.startsWith('cash-review-')?item.id:'';const minute=Math.floor(timestamp(item.at)/60000);const key=explicit||[item.kind,item.reference||'',normalized(item.action),normalized(item.actor),minute].join('|');if(seen.has(key))return false;seen.add(key);return true})}
...
export function UnifiedNotificationHost(){
 ...
 const items=useMemo(()=>{const journalItems=journal.data.map(classify).filter((n):n is ImportantNotification=>Boolean(n));const cashDays=Array.isArray(cashStore.data)?cashStore.data:[];const cashItems=cashDays.map(cashNotification).filter((n):n is ImportantNotification=>Boolean(n));const directionReviewItems=userService==='Direction'?cashDays.map(cashReviewNotification).filter((n):n is ImportantNotification=>Boolean(n)):[];return dedupe([...directionReviewItems,...cashItems,...journalItems]).filter(n=>n.pending||dayKey(timestamp(n.at))===today).sort((a,b)=>Number(Boolean(b.pending))-Number(Boolean(a.pending))||timestamp(b.at)-timestamp(a.at)).slice(0,100)},[journal.data,cashStore.data,today,userService]);
