...
export function UnifiedNotificationHost(){
 ...
 const items=useMemo(()=>dedupe(journal.data.map(classify).filter((n):n is ImportantNotification=>Boolean(n)).sort((a,b)=>timestamp(b.at)-timestamp(a.at))).slice(0,100),[journal.data]);
 const unread=items.filter(i=>!seen.has(i.id));
 const instructionInbox=useMemo(()=>instructions.data.filter(i=>(userService==='Direction'||(Array.isArray(i.recipients)&&i.recipients.some(r=>r.userId===userId)))&&!(i.readBy||[]).some(r=>r.userId===userId)).sort((a,b)=>Date.parse(a.createdAt)-Date.parse(b.createdAt)),[instructions.data,userId,userService]);
 const totalUnread=unread.length+instructionInbox.length;
 useEffect(()=>{if(!userId||!instructionInbox.length)return;const key=`hospicore.instructions.inbox.opened.${userId}`;if(sessionStorage.getItem(key))return;sessionStorage.setItem(key,'1');setOpen(true)},[userId,instructionInbox.length]);
 ...
 <header><div><strong>Boîte de réception</strong><small>Uniquement les actions réelles et informations importantes pour votre compte</small></div><button onClick={()=>setOpen(false)} aria-label="Fermer"><X size={18}/></button></header>
