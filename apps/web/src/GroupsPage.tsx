...
type Group = {
  id: string; name: string; agency: string; arrival: string; departure: string; pax: number; rooms: number;
  ...
  stayType: StayType; breakfastType: BreakfastType; housekeepingType: HousekeepingType; dietary: string;
  luggageArrival: LuggageRoom; luggageDeparture: LuggageRoom; mealDays: MealDay[];
  commercialValidated: boolean; validatedAt: string; validatedBy: string;
  receptionState: ServiceState; restaurantState: ServiceState; housekeepingState: ServiceState;
  kitchenState: ServiceState; commercialState: ServiceState; receptionNotes: string; restaurantNotes: string;
  housekeepingNotes: string; kitchenNotes: string; commercialNotes: string; audit: Audit[];
};
...
function normalize(value: Partial<Group> & { id: string }): Group {
  ...
  return {
    id: value.id, name: value.name || '', agency: value.agency || '', arrival: value.arrival || '', departure: value.departure || '', pax: Number(value.pax || 0), rooms: Number(value.rooms || 0),
    ...
    commercialValidated: Boolean(value.commercialValidated), validatedAt: value.validatedAt || '', validatedBy: value.validatedBy || '', receptionState: value.receptionState || 'À préparer', restaurantState: value.restaurantState || 'À préparer', housekeepingState: value.housekeepingState || 'À préparer', kitchenState: value.kitchenState || 'À préparer', commercialState: value.commercialState || 'En cours',
    receptionNotes: value.receptionNotes || '', restaurantNotes: value.restaurantNotes || '', housekeepingNotes: value.housekeepingNotes || '', kitchenNotes: value.kitchenNotes || '', commercialNotes: value.commercialNotes || '', audit: Array.isArray(value.audit) ? value.audit : [],
  };
}
function permissions(role: string) {
  const value = role.toLowerCase();
  return { commercial: value.includes('commercial') || value.includes('admin') || value.includes('direction'), reception: value.includes('réception') || value.includes('reception') || value.includes('admin') || value.includes('direction') };
}
...
export function GroupsPage() {
  ...
  const rights = permissions(user.role);
  const canEditAll = rights.commercial;
  const canEditTimes = rights.commercial || rights.reception;
  const leaderName = (group: Group) => `${group.leaderFirstName} ${group.leaderLastName}`.trim();
  const filtered = useMemo(() => groups.filter(group => (status === 'Tous' || group.status === status) && `${group.name} ${group.agency} ${leaderName(group)}`.toLowerCase().includes(search.toLowerCase())), [groups, search, status]);
  ...
  async function saveGroup(updated: Group, action: string, invalidateCommercial = false) {
    const shouldInvalidate = invalidateCommercial && updated.commercialValidated;
    const finalGroup: Group = shouldInvalidate ? { ...updated, commercialValidated: false, validatedAt: '', validatedBy: '', commercialState: 'En cours' } : updated;
  ...
  async function updateMeal(date: string, row: MealRow, update: Partial<MealCell>, action: string) {
    if (!selected) return;
    const receptionTimeOnly = rights.reception && !rights.commercial && Object.keys(update).every(key => key === 'time');
  ...
  async function validateCommercial() {
    if (selected && canEditAll) await saveGroup({ ...selected, commercialValidated: true, validatedAt: stamp(), validatedBy: user.name, commercialState: 'Validé' }, 'Fiche verrouillée et validée par le Commercial', false);
  }
  async function create(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!canEditAll) return;
    const form = new FormData(event.currentTarget), pax = Number(form.get('pax') || 0), stayType = String(form.get('stayType') || 'B&B') as StayType, arrival = String(form.get('arrival') || ''), departure = String(form.get('departure') || ''), paymentStatus = String(form.get('paymentStatus') || 'Reste à payer') as PaymentStatus;
    const group = normalize({ id: crypto.randomUUID(), name: String(form.get('name') || ''), agency: String(form.get('agency') || ''), arrival, departure, pax, rooms: Number(form.get('rooms') || 0), status: paymentStatus === 'Payé' ? 'Confirmé' : 'Préparation', paymentStatus, amountDue: Number(form.get('amountDue') || 0), debtor: String(form.get('debtor') || ''), arrivalTime: String(form.get('arrivalTime') || ''), departureTime: String(form.get('departureTime') || ''), leaderFirstName: String(form.get('leaderFirstName') || ''), leaderLastName: String(form.get('leaderLastName') || ''), leaderPhone: String(form.get('leaderPhone') || ''), leaderEmail: String(form.get('leaderEmail') || ''), language: String(form.get('language') || ''), buses: Number(form.get('buses') || 0), stayType, breakfastType: String(form.get('breakfastType') || 'Standard') as BreakfastType, housekeepingType: String(form.get('housekeepingType') || 'Standard') as HousekeepingType, dietary: String(form.get('dietary') || ''), luggageArrival: String(form.get('luggageArrival') || '') as LuggageRoom, luggageDeparture: String(form.get('luggageDeparture') || '') as LuggageRoom, mealDays: buildMealDays(arrival, departure, pax, stayType), audit: [{ id: crypto.randomUUID(), action: 'Fiche Groupe 360° créée', actor: user.name, role: user.role, at: stamp() }] });
  ...
  return <div className="groups-page">
    ...
    <section className="groups-toolbar"><label className="search-box"><Search size={18}/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Rechercher un groupe…"/></label><div className="filter-chips">{(['Tous','Préparation','Confirmé','Arrivé','En séjour','Parti'] as const).map(value => <button key={value} className={`filter-chip${status === value ? ' active' : ''}`} onClick={() => setStatus(value)}>{value}</button>)}</div></section>
    <section className="groups-layout"><div className="groups-table panel"><div className="groups-row groups-head"><span>Groupe</span><span>Séjour</span><span>Pax</span><span>Paiement</span><span>Rooming</span><span>Statut</span></div>{filtered.map(group => <button key={group.id} className={`groups-row${selected?.id === group.id ? ' selected' : ''}`} onClick={() => setSelectedId(group.id)}><span><strong>{group.name}</strong><small>{group.agency}</small></span><span>{group.arrival}<small>→ {group.departure}</small></span><span><strong>{group.pax}</strong><small>{group.rooms} ch.</small></span><span className={`group-pill ${group.paymentStatus === 'Payé' ? 'success' : 'danger'}`}>{group.paymentStatus}</span><span>{group.rooming ? '✅ Reçue' : '❌ Manquante'}</span><span className="group-pill success">{group.status}</span></button>)}</div>
    {selected ? <aside className="group-detail panel group-detail-wide">
      <div className="detail-heading"><div><p className="eyebrow">Fiche Groupe 360°</p><h2>{selected.name}</h2><span>{selected.agency}</span></div><span className="group-code">{selected.id.slice(0,8).toUpperCase()}</span></div>
      ...
      <div className="group-fields">
        ...
        <label>Housekeeping<select disabled={!canEditAll} value={selected.housekeepingType} onChange={event => void patch({ housekeepingType: event.target.value as HousekeepingType }, 'Housekeeping modifié')}><option>Standard</option><option>Prestation express</option></select></label>
        <label>Heure arrivée<input disabled={!canEditTimes} type="time" value={selected.arrivalTime} onChange={event => void patch({ arrivalTime: event.target.value }, 'Heure arrivée modifiée', canEditAll)}/></label>
        <label>Heure départ<input disabled={!canEditTimes} type="time" value={selected.departureTime} onChange={event => void patch({ departureTime: event.target.value }, 'Heure départ modifiée', canEditAll)}/></label>
        <label>Bagagerie arrivée<select disabled={!canEditAll} value={selected.luggageArrival} onChange={event => void patch({ luggageArrival: event.target.value as LuggageRoom }, 'Bagagerie arrivée modifiée')}><option value="">Aucune</option><option>Bagagerie 1</option><option>Bagagerie 2</option><option>Bagagerie 3</option></select></label>
        <label>Bagagerie départ<select disabled={!canEditAll} value={selected.luggageDeparture} onChange={event => void patch({ luggageDeparture: event.target.value as LuggageRoom }, 'Bagagerie départ modifiée')}><option value="">Aucune</option><option>Bagagerie 1</option><option>Bagagerie 2</option><option>Bagagerie 3</option></select></label>
