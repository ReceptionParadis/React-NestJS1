...
type PaymentStatus = 'Payé' | 'Reste à payer' | 'Débiteur';
...
function buildMealDays(arrival: string, departure: string, pax: number, stayType: StayType, previous: MealDay[] = []): MealDay[] {
  ...
  return dateRange(arrival, departure).map((date, index, dates) => {
    ...
    if (prior) rows.forEach(row => { day[row] = { ...day[row], ...prior[row] }; });
    return day;
  });
}
...
export function GroupsPage() {
  ...
  const snapshotRef = useRef({ count: 0, first: '' });
  useEffect(() => {
    const first = groups[0]?.id || '';
    const previous = snapshotRef.current;
    if (first && (!selectedId || (groups.length > previous.count && first !== previous.first))) setSelectedId(first);
    snapshotRef.current = { count: groups.length, first };
  }, [groups, selectedId]);
  ...
  async function saveGroup(updated: Group, action: string, invalidateCommercial = false, immediate = false) {
    ...
    const latest = store.data.map(normalize);
    const payload = latest.map(group => group.id === finalGroup.id ? { ...finalGroup, audit: [...finalGroup.audit, { id: crypto.randomUUID(), action: auditAction, actor: user.name, role: user.role, at: stamp() }] } : group);
    await (immediate ? store.saveImmediate(payload) : store.save(payload));
  }
  ...
  async function updateRooming(received: boolean) {
    ...
    await saveGroup({ ...selected, rooming: received, roomingReceivedAt: receivedAt, roomingReceivedBy: receivedBy }, received ? `Rooming List reçue · ${receivedAt}` : 'Rooming List repassée en statut Manquante', false);
  }
  async function regenerate(update: Partial<Pick<Group, 'arrival' | 'departure' | 'pax' | 'stayType'>> = {}) {
    if (!selected || !canEditAll) return;
    const current = store.data.map(normalize).find(group => group.id === selected.id) || selected;
    const merged = { ...current, ...update };
    if (merged.arrival && merged.departure && merged.departure < merged.arrival) return;
    await saveGroup({ ...merged, mealDays: buildMealDays(merged.arrival, merged.departure, merged.pax, merged.stayType, current.mealDays) }, 'Planning repas recalculé', true, true);
  }
  async function updateMeal(date: string, row: MealRow, update: Partial<MealCell>, action: string) {
    ...
    const current = store.data.map(normalize).find(group => group.id === selected.id) || selected;
    await saveGroup({ ...current, mealDays: current.mealDays.map(day => day.date === date ? { ...day, [row]: { ...day[row], ...update } } : day) }, action, canEditAll, true);
  }
  async function validateCommercial() {
    if (selected && can('commercial.edit', role)) await saveGroup({ ...selected, commercialValidated: true, validatedAt: stamp(), validatedBy: user.name }, 'Fiche verrouillée et validée par le Commercial', false, true);
  }
  ...
  return <div className="groups-page">
    ...
    {selected ? <aside className="group-detail panel group-detail-wide">
      ...
      <div className="group-fields">
        ...
        <label>Date d’arrivée<input disabled={!canEditAll} type="date" value={selected.arrival} max={selected.departure || undefined} onChange={event => void regenerate({ arrival: event.target.value })}/></label>
        <label>Date de départ<input disabled={!canEditAll} type="date" value={selected.departure} min={selected.arrival || undefined} onChange={event => void regenerate({ departure: event.target.value })}/></label>
        ...
        <label>Paiement<select disabled={!canEditAll} value={selected.paymentStatus} onChange={event => void patch({ paymentStatus: event.target.value as PaymentStatus }, 'Paiement modifié')}><option>Payé</option><option>Reste à payer</option><option>Débiteur</option></select></label>
