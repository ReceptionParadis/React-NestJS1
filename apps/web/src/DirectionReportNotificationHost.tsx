...
function DirectionReportNotificationInner({userId}:{userId:string}){
 const store=useOperationalStore<Report[]>('direction-daily-reports',[],5000),[seen,setSeen]=useState<Set<string>>(()=>loadSeen(userId)),[toast,setToast]=useState<Report|null>(null),initialized=useRef(false);
 const assigned=useMemo(()=>store.data.filter(r=>!r.recipientIds?.length||r.recipientIds.includes(userId)).sort((a,b)=>b.date.localeCompare(a.date)),[store.data,userId]),unread=assigned.filter(r=>!seen.has(r.id));
 useEffect(()=>{if(store.state==='loading')return;if(!initialized.current){initialized.current=true;if(unread[0])setToast(unread[0]);return}if(unread[0])setToast(current=>current?.id===unread[0].id?current:unread[0])},[store.version,unread.length]);
...
export function DirectionReportNotificationHost(){const s=session(),u=s.user||{},userId=String(u.id||'');if(!s.token||!userId||roleFromValue(u.role)!=='direction')return null;return <DirectionReportNotificationInner userId={userId}/>}
