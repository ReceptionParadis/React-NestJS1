...
export function MealTransmissionDashboard(){
 ...
 const pending=useMemo(()=>store.data.filter(o=>!o.verifiedAt||!o.printedAt).sort((a,b)=>`${a.date}${a.pickupTime}`.localeCompare(`${b.date}${b.pickupTime}`)),[store.data]);
 useEffect(()=>{
  if(window.location.pathname!=='/'&&window.location.pathname!=='')return;
  let node=document.getElementById('meal-transmission-dashboard-mount') as HTMLElement|null;
  const attach=()=>{
   const host=document.querySelector('.executive-main');if(!host)return false;
   if(!node){node=document.createElement('div');node.id='meal-transmission-dashboard-mount';const metrics=host.querySelector('.executive-metrics');metrics?.after(node);if(!metrics)host.appendChild(node);}
 ...
 return createPortal(<section className={`meal-transmission-dashboard ${pending.length?'has-pending':'is-clear'}`}>
  <header><div className="meal-transmission-title"><span className="meal-transmission-icon"><PackageCheck size={22}/></span><div><p>Réception · Bons de commande</p><h2>Bons en attente de transmission</h2><small>Un bon reste affiché tant qu’il n’est pas vérifié et imprimé.</small></div></div><button onClick={()=>location.assign('/reception/paniers-repas-pdj')}>Ouvrir les bons <ExternalLink size={15}/></button></header>
  ...
  {pending.length===0?<div className="meal-transmission-empty"><CheckCircle2 size={24}/><div><strong>Tout est transmis</strong><span>Aucun bon n’attend de vérification ou d’impression.</span></div></div>:<div className="meal-transmission-list">{pending.slice(0,8).map(o=><article key={o.id}><div className="meal-transmission-main"><strong>{o.type} · {o.quantity} unité(s)</strong><span>{o.recipient||'Groupe'} · {dateLabel(o.date)} · {o.pickupTime||'—'}</span></div><div className="meal-transmission-statuses"><span className={o.verifiedAt?'done':'pending'}><ShieldCheck size={13}/>{o.verifiedAt?'Vérifié':'À vérifier'}</span><span className={o.printedAt?'done':'pending'}><Printer size={13}/>{o.printedAt?'Imprimé':'À imprimer'}</span></div>{!o.verifiedAt&&<button className="meal-verify-button" onClick={()=>void verify(o.id)}><CheckCircle2 size={14}/>Valider la vérification</button>}</article>)}</div>}
