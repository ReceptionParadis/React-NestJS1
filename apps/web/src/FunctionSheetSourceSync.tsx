...
function groupToLine(g:Group,prior?:FunctionLine):FunctionLine{const deposit=Boolean(g.depositReceived||g.paymentStatus==='Payé'),leader=leaderOf(g),validatedAt=new Date().toLocaleString('fr-FR');return{id:prior?.id||crypto.randomUUID(),groupId:g.id,commercial:creatorOf(g),groupName:g.name||'',nationality:g.nationality||g.language||'',stayType:g.stayType||'',groupStatus:deposit?'Confirmé':g.status||'Préparation',remarks:g.commercialNotes||'',leaderFirstName:leader.first,leaderLastName:leader.last,leaderPhone:leader.phone,leaderEmail:leader.email,arrivalDate:g.arrival||'',arrivalTime:g.arrivalTime||'',arrivalService:firstService(g),departureDate:g.departure||'',departureTime:g.departureTime||'',lastService:lastService(g),roomingReceived:Boolean(g.rooming),luggageArrival:g.luggageArrival||'',luggageDeparture:g.luggageDeparture||'',paymentStatus:g.paymentStatus||'',amountDue:Number(g.amountDue||0),debtor:g.debtor||'',depositReceived:deposit,breakfastType:g.breakfastType||'Standard',mealSummary:mealSummary(g),dietary:g.dietary||g.allergies||'',roomService:g.housekeepingType||'Standard',earlyCheckIn:before(g.arrivalTime||'','16:00')?`Oui · ${g.arrivalTime}`:'Non',lateCheckOut:after(g.departureTime||'','09:00')?`Oui · ${g.departureTime}`:'Non',lineStatus:'Validée',validatedBy:'HospiCore · Groupe 360°',validatedAt,departureConfirmed:Boolean(prior?.departureConfirmed)}}
function overlaps(g:Group,sheet:WeeklySheet){return String(g.arrival||'')<=sheet.weekEnd&&String(g.departure||'')>=sheet.weekStart}
function relevant(groups:Group[],sheet:WeeklySheet){const existing=new Set((sheet.lines||[]).map(line=>line.groupId));return groups.filter(g=>(g.commercialValidated||existing.has(g.id))&&overlaps(g,sheet)).sort((a,b)=>String(a.id).localeCompare(String(b.id)))}
...
function fingerprint(groups:Group[],sheet:WeeklySheet){const existing=new Map((sheet.lines||[]).map(line=>[line.groupId,line]));return JSON.stringify(relevant(groups,sheet).map(group=>group.commercialValidated?groupProjection(group):{id:group.id,pendingRevalidation:true,previous:existing.get(group.id)?.validatedAt||''}))}
...
export function FunctionSheetSourceSync(){
 ...
 useEffect(()=>{
  ...
  const next=sheets.data.map(sheet=>{
   ...
   const needsMigration=sheet.status!=='Prête à imprimer'||(sheet.lines||[]).some(line=>line.lineStatus!=='Validée');
   if(sheet.sourceFingerprint===fp&&!needsMigration)return sheet;
   changed=true;
   const old=new Map((sheet.lines||[]).map(line=>[line.groupId,line]));
   const lines=relevant(groups.data,sheet).map(group=>{
    const prior=old.get(group.id);
    if(!group.commercialValidated&&prior)return{...prior,lineStatus:'Validée' as const,validatedBy:prior.validatedBy||'HospiCore · dernière version validée',validatedAt:prior.validatedAt||printedAt};
    return groupToLine(group,prior);
   }).sort((a,b)=>`${a.arrivalDate||'9999-12-31'}T${a.arrivalTime||'23:59'}`.localeCompare(`${b.arrivalDate||'9999-12-31'}T${b.arrivalTime||'23:59'}`)||a.groupName.localeCompare(b.groupName,'fr',{sensitivity:'base'}));
   return{...sheet,lines,status:'Prête à imprimer' as const,validatedForPrintBy:'HospiCore · Groupe 360°',validatedForPrintAt:printedAt,lockedBy:'',lockedAt:'',sourceFingerprint:fp,lastSourceSyncAt:now};
  });
  if(!changed)return;
