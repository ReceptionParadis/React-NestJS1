...
const metrics = [
  ...
  { label: 'Transmissions', value: '8', detail: '4 prioritaires', delta: '5 services connectés', tone: 'blue' },
...
export function App() {
  ...
  return <div className="app-shell executive-shell">
    ...
    <main className="main-content executive-main">
      ...
      <section className="executive-hero"><div><p className="eyebrow light">Briefing HospiCore</p><h2>Une seule information, partagée avec tous les services.</h2><p>Le Centre des opérations centralise les prêts et l’inventaire. Les consignes générales regroupent bagagerie, colis, taxis, VIP, incidents et demandes spéciales.</p></div><div className="hero-actions"><button onClick={() => navigate('/consignes-generales')}><ClipboardList size={18}/>Consignes générales</button><button onClick={() => navigate('/centre-operations')}><BookOpenCheck size={18}/>Prêts & inventaire</button></div></section>
      ...
      <section className="executive-grid">
        ...
        <article className="panel quick-panel"><div className="panel-header compact"><div><p className="eyebrow">Accès rapide</p><h2>Modules opérationnels</h2></div></div><div className="quick-grid">{nav.slice(1,8).map(({label,icon:Icon,href}) => <button key={label} onClick={() => navigate(href)}><Icon size={22}/><span>{label}</span><ChevronRight size={16}/></button>)}</div></article>
      </section>
