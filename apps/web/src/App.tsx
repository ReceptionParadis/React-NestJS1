...
import {
  AlertTriangle, BedDouble, Bell, CalendarDays, CheckCircle2, ChefHat, ChevronRight,
  ClipboardList, Clock3, LayoutDashboard, Menu, MessageSquareText, Sparkles,
  UsersRound, Wrench, X,
} from 'lucide-react';
...
const nav = [
  ...
  { label: 'Planning Restaurant', icon: ChefHat, href: '/restaurant' },
...
const metrics = [
  ...
  { label: 'Couverts groupes', value: '175', detail: '3 services aujourd’hui', delta: '3 groupes non arrivés', tone: 'orange' },
...
const arrivals = [
  ...
  { time: '16h00', name: 'Marian Pilgrimages', meta: '54 pax · dîner 19h00', state: 'Prêt', tone: 'warning' },
  { time: '17h45', name: 'Unitalsi', meta: '82 pax · dîner 19h00', state: 'En route', tone: 'info' },
  { time: '19h15', name: 'Joe Walsh Tours', meta: '31 pax · dîner 19h30', state: 'Non arrivé', tone: 'danger' },
];

const alerts = [
  { icon: Wrench, title: 'Ascenseur bâtiment B', text: 'Intervention prévue à 14h00', tone: 'critical' },
  { icon: ChefHat, title: '3 groupes non arrivés', text: 'Restaurant : dîner à partir de 19h00', tone: 'warning' },
...
const services = [
  ...
  { name: 'Restaurant', status: 'À suivre', detail: '3 groupes attendus au dîner', score: 82 },
];
...
export function App() {
  ...
  return <div className="app-shell executive-shell">
    ...
    <main className="main-content executive-main">
      <header className="topbar executive-topbar"><div className="heading-wrap"><button className="mobile-menu" onClick={() => setSidebarOpen(true)} aria-label="Menu"><Menu size={22}/></button><div><p className="eyebrow">Mercredi 5 août 2026 · 16h28</p><h1>Centre opérationnel</h1></div></div><div className="topbar-actions"><span className="live-badge"><span/> Données de démonstration</span><button className="icon-button" aria-label="Notifications"><Bell size={20}/><span className="notification-dot">4</span></button><div className="avatar">TP</div></div></header>

      <section className="executive-hero"><div><p className="eyebrow light">Briefing HospiCore AI</p><h2>Une journée dense, mais maîtrisable.</h2><p>Priorité à la coordination des arrivées, aux horaires de repas et à la transmission des informations entre la réception et le restaurant.</p></div><div className="hero-actions"><button onClick={() => navigate('/restaurant')}><ChefHat size={18}/>Planning Restaurant</button><button onClick={() => navigate('/salles-reunion')}><CalendarDays size={18}/>Agenda des salles</button></div></section>
      ...
      <section className="executive-grid">
        <article className="panel arrivals-panel"><div className="panel-header compact"><div><p className="eyebrow">Flux du jour</p><h2>Prochaines arrivées groupes</h2></div><button className="text-button" onClick={() => navigate('/restaurant')}>Voir les repas</button></div><div className="arrival-list">{arrivals.map((arrival) => <button className="arrival-row" key={arrival.name} onClick={() => navigate('/restaurant')}><time>{arrival.time}</time><div><strong>{arrival.name}</strong><small>{arrival.meta}</small></div><span className={`pill ${arrival.tone}`}>{arrival.state}</span><ChevronRight size={17}/></button>)}</div></article>
        <article className="panel alerts-panel"><div className="panel-header compact"><div><p className="eyebrow">À traiter</p><h2>Alertes prioritaires</h2></div><span className="count-badge">4</span></div><div className="executive-alerts">{alerts.map(({icon:Icon,title,text,tone}) => <div className={`executive-alert ${tone}`} key={title}><span><Icon size={18}/></span><div><strong>{title}</strong><small>{text}</small></div></div>)}</div></article>
        ...
        <article className="panel quick-panel"><div className="panel-header compact"><div><p className="eyebrow">Accès rapide</p><h2>Modules de la démo</h2></div></div><div className="quick-grid">{nav.slice(1).map(({label,icon:Icon,href}) => <button key={label} onClick={() => navigate(href)}><Icon size={22}/><span>{label}</span><ChevronRight size={16}/></button>)}<button><BedDouble size={22}/><span>Housekeeping</span><em>Bientôt</em></button></div></article>
      </section>
