...
import {
  ...
  ClipboardList, Clock3, ConciergeBell, History, LayoutDashboard, Menu, MessageSquareText, Sparkles,
...
export function App() {
  ...
  return <div className="app-shell executive-shell">
    ...
    <main className="main-content executive-main">
      ...
      <section className="executive-hero"><div><p className="eyebrow light">Briefing HospiCore</p><h2>Chaque action est désormais visible et traçable.</h2><p>Le Journal d’exploitation regroupe les mises à jour signées des groupes, consignes, prêts, repas et opérations interservices.</p></div><div className="hero-actions"><button onClick={() => navigate('/journal-exploitation')}><History size={18}/>Journal d’exploitation</button><button onClick={() => navigate('/consignes-generales')}><ClipboardList size={18}/>Consignes générales</button></div></section>
      ...
      <section className="executive-grid">
        ...
        <article className="panel quick-panel"><div className="panel-header compact"><div><p className="eyebrow">Accès rapide</p><h2>Modules opérationnels</h2></div></div><div className="quick-grid">{nav.slice(1,9).map(({label,icon:Icon,href}) => <button key={label} onClick={() => navigate(href)}><Icon size={22}/><span>{label}</span><ChevronRight size={16}/></button>)}</div></article>
      </section>
