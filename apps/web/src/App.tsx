...
function checklist(group:Group){
 const c=group.departureChecklist;
 const payment=c?.settlementStatus==='paid'?Boolean(c.paymentMethod):c?.settlementStatus==='debtor'?Boolean(c.debtorName):false;
 const keys=Boolean(c?.keysRecovered);
...
export function App(){
 ...
 const[receptionMenuOpen,setReceptionMenuOpen]=useState(()=>window.location.pathname.startsWith('/reception'));
 ...
 const session=readSession(),today=isoDate(new Date()),appRole=roleFromValue(session.user?.role);
 const currentRole=normalizedRole(session);
 const visibleNav=nav.filter(item=>canAccessPath(item.href,appRole));
 const receptionVisible=canAccessPath('/reception',appRole);
 ...
 const actions=useMemo<ActionItem[]>(()=>[
  ...blockedDepartures.map(g=>({id:`dep-${g.id}`,label:`Départ bloqué · ${g.name||'Groupe'}`,detail:'Solde, clés ou contrôle à compléter',href:'/reception/arrivees-departs',level:'urgent' as const,category:'reception' as const})),
  ...
  ...controlsToDo.map(g=>({id:`ctl-${g.id}`,label:`Contrôle Groupe · ${g.name||'Groupe'}`,detail:'À réaliser par la Réception',href:'/reception/controles',level:'warning' as const,category:'reception' as const})),
 ...
 const stores=[groupsStore,roomsStore,maintenanceStore];
 const loading=stores.some(s=>s.state==='loading'||s.state==='saving');
 const reportedError=stores.find(s=>s.rawState==='error'||s.rawState==='conflict');
 ...
 const refresh=()=>{void groupsStore.refresh();void roomsStore.refresh();void maintenanceStore.refresh()};
 return <div className="app-shell executive-shell command-shell">
  <aside className={`sidebar${sidebarOpen?' open':''}`}>
   ...
   <nav className="nav-list" aria-label="Navigation principale">
    ...
    {receptionVisible&&<div className={`nav-group reception-nav-group${receptionMenuOpen?' open':''}`}>
      <button data-nav="/reception" className={`nav-item nav-parent${window.location.pathname.startsWith('/reception')?' active':''}`} onClick={()=>setReceptionMenuOpen(v=>!v)} aria-expanded={receptionMenuOpen}><ConciergeBell size={19}/><span>Réception</span>{receptionMenuOpen?<ChevronUp className="nav-chevron" size={17}/>:<ChevronDown className="nav-chevron" size={17}/>}</button>
      {receptionMenuOpen&&<div className="nav-submenu">{receptionSubmenu.map(({label,icon:Icon,href})=><button key={href} className={`nav-subitem${window.location.pathname===href?' active':''}`} onClick={()=>location.assign(href)}><Icon size={16}/><span>{label}</span></button>)}</div>}
    </div>}
    {visibleNav.map(({label,icon:Icon,href})=><button key={href} data-nav={href} className="nav-item" onClick={()=>location.assign(href)}><Icon size={19}/><span>{label}</span></button>)}
   </nav>
  ...
  <main className="live-v2-main command-main">
   ...
   <div className="command-content">
    <section className="command-alerts">
     {canAccessPath('/reception',appRole)&&<button className={blockedDepartures.length?'critical':'ok'} onClick={()=>location.href='/reception/arrivees-departs'}><AlertTriangle size={18}/><strong>{blockedDepartures.length}</strong><span>départ(s) bloqué(s)</span></button>}
     {canAccessPath('/reception',appRole)&&<button className={controlsToDo.length?'warning':'ok'} onClick={()=>location.href='/reception/controles'}><ClipboardCheck size={18}/><strong>{controlsToDo.length}</strong><span>contrôle(s) à réaliser</span></button>}
     {canAccessPath('/tickets',appRole)&&<button className={maintenanceUrgent.length||maintenanceBlocked.length?'critical':'ok'} onClick={()=>location.href='/tickets'}><Wrench size={18}/><strong>{maintenanceUrgent.length+maintenanceBlocked.length}</strong><span>alerte(s) maintenance</span></button>}
     {canAccessPath('/diagnostic',appRole)&&<button className={syncError?'critical':'ok'} onClick={()=>location.href='/diagnostic'}><RefreshCw size={18}/><strong>{syncError?'!':'✓'}</strong><span>{syncError?'synchronisation en erreur':'PostgreSQL synchronisé'}</span></button>}
    </section>
