...
const alerts = [
  { icon: BookOpenCheck, title: 'Fauteuil roulant à restituer', text: 'Retour prévu aujourd’hui à 10h00', tone: 'critical' },
...
export function App() {
  ...
  return <div className="app-shell executive-shell">
    <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}><div className="brand"><div className="brand-mark">H</div><div><strong>HospiCore</strong><span>Hôtel Paradis · Lourdes</span></div><button className="sidebar-close" onClick={() => setSidebarOpen(false)} aria-label="Fermer"><X size={20}/></button></div><nav className="nav-list">{nav.map(({ label, icon: Icon, href, active }) => <button key={label} className={`nav-item${active ? ' active' : ''}`} onClick={() => navigate(href)}><Icon size={19}/>{label}</button>)}</nav><div className="demo-version"><Sparkles size={16}/><div><strong>Executive Demo</strong><span>HospiCore v0.4</span></div></div></aside>
    <main className="main-content executive-main">
      <header className="topbar executive-topbar"><div className="heading-wrap"><button className="mobile-menu" onClick={() => setSidebarOpen(true)} aria-label="Menu"><Menu size={22}/></button><div><p className="eyebrow">Mercredi 5 août 2026 · 22h00</p><h1>Centre opérationnel interservice</h1></div></div><div className="topbar-actions"><span className="live-badge"><span/> Données de démonstration</span><button className="icon-button" aria-label="Notifications"><Bell size={20}/><span className="notification-dot">4</span></button><div className="avatar">TP</div></div></header>
      <section className="executive-hero"><div><p className="eyebrow light">Briefing HospiCore</p><h2>Une seule information, partagée avec tous les services.</h2><p>Le Centre des opérations centralise désormais les consignes et les prêts de matériel, avec une signature et un horodatage automatiques.</p></div><div className="hero-actions"><button onClick={() => navigate('/centre-operations')}><BookOpenCheck size={18}/>Cahier de consignes</button><button onClick={() => navigate('/commercial')}><BriefcaseBusiness size={18}/>Fiches de fonction</button></div></section>
      ...
      <section className="executive-grid">
        ...
        <article className="panel quick-panel"><div className="panel-header compact"><div><p className="eyebrow">Accès rapide</p><h2>Modules opérationnels</h2></div></div><div className="quick-grid">{nav.slice(1,7).map(({label,icon:Icon,href}) => <button key={label} onClick={() => navigate(href)}><Icon size={22}/><span>{label}</span><ChevronRight size={16}/></button>)}</div></article>
      </section>
