...
export function App(){
 ...
 const actions=useMemo<ActionItem[]>(()=>[
  ...blockedDepartures.map(g=>({id:`dep-${g.id}`,label:`Départ bloqué · ${g.name||'Groupe'}`,detail:'Solde, clés ou contrôle à compléter',href:'/reception/controles',level:'urgent' as const,category:'reception' as const})),
  ...
  <main className="live-v2-main command-main">
   ...
   <div className="command-content">
    <section className="command-alerts">
     {canAccessPath('/reception',appRole)&&<button className={blockedDepartures.length?'critical':'ok'} onClick={()=>location.href='/reception/controles'}><AlertTriangle size={18}/><strong>{blockedDepartures.length}</strong><span>départ(s) bloqué(s)</span></button>}
