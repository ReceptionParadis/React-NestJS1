...
import {
  AlertTriangle, BedDouble, Bell, BriefcaseBusiness, CalendarDays, CheckCircle2, ChefHat, ChevronRight,
  ClipboardList, Clock3, ConciergeBell, LayoutDashboard, Menu, MessageSquareText, Sparkles,
  UtensilsCrossed, UsersRound, Wrench, X,
} from 'lucide-react';
...
const metrics = [
  { label: 'Groupes attendus', value: '9', detail: '3 arrivés · 4 en route', delta: '2 fiches à compléter', tone: 'wine' },
  { label: 'Couverts groupes', value: '175', detail: '3 services aujourd’hui', delta: 'Prochain service 19h00', tone: 'gold' },
  { label: 'Fiches de fonction', value: '12', detail: 'Semaine du 3 au 9 août', delta: '10 validées', tone: 'blue' },
  { label: 'Salles réservées', value: '7', detail: '3 événements aujourd’hui', delta: '1 option à confirmer', tone: 'green' },
  { label: 'Transmissions', value: '8', detail: '4 prioritaires', delta: '5 services connectés', tone: 'orange' },
...
const alerts = [
  { icon: ChefHat, title: '3 groupes au dîner', text: 'Restaurant et cuisine à partir de 19h00', tone: 'warning' },
  { icon: BriefcaseBusiness, title: '2 fiches incomplètes', text: 'Horaires repas à confirmer par le commercial', tone: 'warning' },
...
const services = [
  ...
  { name: 'Restaurant', status: 'À suivre', detail: '175 couverts groupes', score: 82 },
  { name: 'Housekeeping', status: 'Informé', detail: '2 priorités groupes', score: 78 },
...
export function App() {
  ...
  return <div className="app-shell executive-shell">
    <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}><div className="brand"><div className="brand-mark">H</div><div><strong>HospiCore</strong><span>Hôtel Paradis · Lourdes</span></div><button className="sidebar-close" onClick={() => setSidebarOpen(false)} aria-label="Fermer"><X size={20}/></button></div><nav className="nav-list">{nav.map(({ label, icon: Icon, href, active }) => <button key={label} className={`nav-item${active ? ' active' : ''}`} onClick={() => navigate(href)}><Icon size={19}/>{label}</button>)}</nav><div className="demo-version"><Sparkles size={16}/><div><strong>Executive Demo</strong><span>HospiCore v0.3</span></div></div></aside>
    <main className="main-content executive-main">
      <header className="topbar executive-topbar"><div className="heading-wrap"><button className="mobile-menu" onClick={() => setSidebarOpen(true)} aria-label="Menu"><Menu size={22}/></button><div><p className="eyebrow">Mercredi 5 août 2026 · 19h20</p><h1>Centre opérationnel interservice</h1></div></div><div className="topbar-actions"><span className="live-badge"><span/> Données de démonstration</span><button className="icon-button" aria-label="Notifications"><Bell size={20}/><span className="notification-dot">4</span></button><div className="avatar">TP</div></div></header>
      <section className="executive-hero"><div><p className="eyebrow light">Briefing HospiCore</p><h2>Une seule information, partagée avec tous les services.</h2><p>Les fiches de fonction commerciales alimentent la réception, le restaurant, la cuisine et le housekeeping. Une arrivée confirmée est immédiatement visible par les équipes concernées.</p></div><div className="hero-actions"><button onClick={() => navigate('/reception')}><ConciergeBell size={18}/>Interface Réception</button><button onClick={() => navigate('/commercial')}><BriefcaseBusiness size={18}/>Fiches de fonction</button></div></section>
      ...
      <section className="executive-grid">
        ...
        <article className="panel quick-panel"><div className="panel-header compact"><div><p className="eyebrow">Espaces métiers</p><h2>Accès par service</h2></div></div><div className="quick-grid">{nav.slice(1,6).map(({label,icon:Icon,href}) => <button key={label} onClick={() => navigate(href)}><Icon size={22}/><span>{label}</span><ChevronRight size={16}/></button>)}</div></article>
      </section>
      <footer className="demo-footer"><AlertTriangle size={16}/>Version pilote : les données sont partagées dans le navigateur. La synchronisation PostgreSQL multi-utilisateur sera la prochaine étape.</footer>
    </main>
  </div>;
}
