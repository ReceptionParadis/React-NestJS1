import { ArrowLeft, BellRing, CalendarDays, ClipboardCheck, FileArchive, FileText, LogIn, MoonStar, UsersRound } from 'lucide-react';
...
export function ReceptionHubPage(){
 const groups=useOperationalStore<Group[]>('group-360',[]),sheets=useOperationalStore<WeeklySheet[]>('function-sheets',[]),requests=useOperationalStore<IndividualRequest[]>('individual-requests',[]),nightRoutes=useOperationalStore<NightRoute[]>('night-route-notes',[]);
 ...
 const cards=[
  ...
  {title:'Demandes clients individuels',description:'Enregistrer taxis, réveils, PDJ Box et paniers repas avec chambre, horaire et consignes.',href:'/reception/demandes-individuelles',icon:BellRing,count:activeRequests,label:'demande(s) active(s)'},
  ...
  {title:'Feuille de route veilleur',description:'Consolider les événements de 22h00 à 08h30 et imprimer la feuille de nuit en A4 portrait.',href:'/reception/feuille-route-veilleur',icon:MoonStar,count:nightRoutes.data.filter(n=>n.date===today).length,label:'feuille de nuit'},
  {title:'Archives',description:'Retrouver contrôles Groupe, fiches hebdomadaires et feuilles de nuit validés, classés par mois.',href:'/reception/archives',icon:FileArchive,count:archivedControls+archivedSheets+archivedNights,label:'document(s) archivé(s)'},
 ];
 return <main className="reception-hub-page"><header className="reception-hub-header"><button onClick={()=>location.href='/'}><ArrowLeft size={18}/>Centre de Commandement</button><p>HospiCore · Réception</p><h1><FileText size={31}/>Espace Réception</h1><span>Chaque carte ouvre directement le module demandé, sans repasser par un tableau intermédiaire.</span></header><section className="reception-hub-grid">{cards.map(({title,description,href,icon:Icon,count,label})=><button key={href} onClick={()=>location.href=href}><div className="reception-hub-icon"><Icon size={28}/></div><div className="reception-hub-card-content"><div><h2>{title}</h2><p>{description}</p></div><footer><strong>Ouvrir</strong><span><b>{count}</b> {label}</span></footer></div></button>)}</section></main>
}
