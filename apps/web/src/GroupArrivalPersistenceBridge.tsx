 ...
 useEffect(()=>{
  ...
  const onClick=(event:MouseEvent)=>{
   ...
   void fetch(`/api/operational-sync/group-360?${params.toString()}`,{headers,cache:'no-store'})
    ...
    .then(async response=>{const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data?.message||`Arrivée impossible (${response.status})`);const version=Number(data?.version||0);window.dispatchEvent(new CustomEvent('hospicore:operational-change',{detail:{namespace:'group-360',version,at:Date.now(),source:'remote'}}));})
