...
export function currentRole(): AppRole {
  try { return roleFromValue(JSON.parse(localStorage.getItem('hospicore.session') || '{}')?.user?.role); }
...
const matrix: Record<AppRole, ReadonlySet<Capability>> = {
  direction: new Set<Capability>([
    ...
    'commercial.view','commercial.edit','commercial.validate-control','maintenance.view','maintenance.create','maintenance.manage',
    'planning.view','meeting-rooms.view','meeting-rooms.edit','tasks.view','tasks.edit','instructions.view','instructions.edit',
    'operations-center.view','operations-center.edit','journal.view','diagnostic.view','administration.view',
  ]),
  ...
  reception: new Set<Capability>([
    'dashboard.view','reception.view','reception.operate','group-control.create','maintenance.view','maintenance.create',
    'planning.view','meeting-rooms.view','meeting-rooms.edit','tasks.view','tasks.edit','instructions.view',
    'operations-center.view','operations-center.edit','journal.view',
  ]),
  ...
  maintenance: new Set<Capability>([
    'dashboard.view','maintenance.view','maintenance.create','maintenance.manage','planning.view','meeting-rooms.view','tasks.view','tasks.edit','instructions.view',
  ]),
  unknown: new Set<Capability>(['dashboard.view']),
};

export function can(capability: Capability, role: AppRole = currentRole()) { return matrix[role].has(capability); }
